...
export default function ArtistCommissionsPage() {
  ...
  async function respond(requestId: string, decision: "accepted" | "rejected") {
    ...
    try {
      ...
      const body = await response.json();
      if (!response.ok || !body.ok) throw new Error(body.message ?? t("artistDashboard.commissions.unableToRespond"));
      setMessage(decision === "accepted" ? t("artistDashboard.commissions.commissionAccepted") : t("artistDashboard.commissions.commissionRejected"));
      await loadRequests();
    } catch (error) {
      setError(error instanceof Error ? error.message : t("artistDashboard.commissions.unableToRespond"));
    }
  }

  return (
    <DashboardLayout role="artist" userName={t("artistDashboard.commissions.userName")}>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t("artistDashboard.commissions.heading")}</h1>
          <p className="text-gray-500">{t("artistDashboard.commissions.subheading")}</p>
        </div>
        ...
        <div className="space-y-4">
              ...
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                <div>
                  ...
                  <p className="mt-2 text-sm text-gray-500">
                    {t("artistDashboard.commissions.budgetSize", {
                      budget: request.budgetAmount.toLocaleString(),
                      currency: request.currency,
                      size: request.dimensions ?? request.sizeCategory,
                    })}
                  </p>
                  {request.preferredMaterials && <p className="text-sm text-gray-500">{t("artistDashboard.commissions.preferredMaterials", { materials: request.preferredMaterials })}</p>}
                  {request.adminNotes && <p className="text-sm text-blue-700">{t("artistDashboard.commissions.adminNote", { note: request.adminNotes })}</p>}
                  {request.artistResponseNote && <p className="text-sm text-gray-500">{t("artistDashboard.commissions.yourNote", { note: request.artistResponseNote })}</p>}
                </div>
              ...
              {request.status === "assigned" && (
                <div className="space-y-3">
                  <textarea
                    ...
                    placeholder={t("artistDashboard.commissions.responseNotePlaceholder")}
                    rows={3}
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <button onClick={() => respond(request.id, "accepted")} className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-teal-600 text-white rounded-lg hover:bg-teal-700">
                      ...
                      {t("artistDashboard.commissions.acceptProject")}
                    </button>
                    <button onClick={() => respond(request.id, "rejected")} className="inline-flex items-center justify-center gap-2 px-4 py-2.5 border border-red-200 text-red-600 rounded-lg hover:bg-red-50">
                      ...
                      {t("artistDashboard.commissions.reject")}
                    </button>
                  </div>
                </div>
              )}
          ...
          {requests.length === 0 && (
            <div className="bg-white border border-gray-100 rounded-xl p-8 text-center text-gray-500">
              {t("artistDashboard.commissions.emptyState")}
            </div>
          )}
        </div>
      </div>
