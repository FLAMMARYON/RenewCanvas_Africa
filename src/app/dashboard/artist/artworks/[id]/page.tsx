...
export default function EditArtworkPage() {
  ...
  const [userName, setUserName] = useState(t("artistDashboard.edit.defaultUserName"));

  useEffect(() => {
    readProfile()
      .then((profile) => setUserName(profile.displayName || t("artistDashboard.edit.defaultUserName")))
      .catch(() => {});
    readArtwork(params.id)
      ...
      .catch((error) => setStatusMessage(error instanceof Error ? error.message : t("artistDashboard.edit.loadError")));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setStatusMessage(error instanceof Error ? error.message : t("artistDashboard.edit.saveError"));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleEvidenceSubmit = async () => {
    ...
    try {
      ...
      setEvidenceLabel("");
      setEvidenceNotes("");
      setStatusMessage(t("artistDashboard.edit.evidenceSubmitted"));
    } catch (error) {
      setStatusMessage(error instanceof Error ? error.message : t("artistDashboard.edit.evidenceError"));
    }
  };
    ...
    return (
      <DashboardLayout role="artist" userName={userName}>
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          {statusMessage || t("artistDashboard.edit.loading")}
        </div>
      </DashboardLayout>
    );
  }
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">{t("artistDashboard.edit.title")}</h1>
              <p className="text-gray-500">{t("artistDashboard.edit.subtitle")}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleSave} disabled={isSaving} className="inline-flex items-center gap-2 rounded-lg bg-teal-600 px-5 py-2.5 text-white hover:bg-teal-700 disabled:opacity-50">
              ...
              {isSaving ? t("artistDashboard.edit.saving") : saveSuccess ? t("artistDashboard.edit.saved") : t("artistDashboard.edit.saveChanges")}
            </button>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="space-y-6 lg:col-span-2">
            <section className="rounded-xl border border-gray-100 bg-white p-6">
              <h2 className="mb-4 font-semibold text-gray-900">{t("artistDashboard.edit.basicInformation")}</h2>
              <div className="space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <input value={artwork.dimensions ?? ""} onChange={(event) => handleInputChange("dimensions", event.target.value)} placeholder={t("artistDashboard.edit.dimensionsPlaceholder")} className="rounded-lg border border-gray-200 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-teal-500" />
                </div>
              </div>
            </section>
            ...
            <section className="rounded-xl border border-gray-100 bg-white p-6">
              <div className="mb-4 flex items-center justify-between">
                <h2 className="flex items-center gap-2 font-semibold text-gray-900">
                  ...
                  {t("artistDashboard.edit.recycledMaterials")}
                </h2>
                <button onClick={() => setMaterials((current) => [...current, { material: "", weightKg: 0, source: "" }])} className="inline-flex items-center gap-1 rounded-lg px-3 py-1.5 text-sm text-teal-600 hover:bg-teal-50">
                  ...
                  {t("artistDashboard.edit.addMaterial")}
                </button>
              </div>
                ...
                {materials.map((material, index) => (
                  <div key={index} className="grid gap-3 rounded-lg bg-gray-50 p-4 sm:grid-cols-[1fr_120px_1fr_auto]">
                    <select value={material.material} onChange={(event) => handleMaterialChange(index, "material", event.target.value)} className="rounded-lg border border-gray-200 bg-white px-3 py-2">
                      <option value="">{t("artistDashboard.edit.selectMaterial")}</option>
                    ...
                    <input value={material.source} onChange={(event) => handleMaterialChange(index, "source", event.target.value)} placeholder={t("artistDashboard.edit.sourcePlaceholder")} className="rounded-lg border border-gray-200 px-3 py-2" />
              ...
              <div className="mt-4 rounded-lg bg-green-50 p-4 text-green-700">
                {t("artistDashboard.edit.totalWasteDiverted")} <strong>{totalWeight.toFixed(1)} kg</strong>
              </div>
            </section>
          </div>
          ...
          <aside className="space-y-6">
            {verificationRequest && (
              <section className="rounded-xl border border-amber-100 bg-white p-6">
                <h2 className="mb-2 font-semibold text-gray-900">{t("artistDashboard.edit.verificationRequest")}</h2>
                <p className="text-sm text-amber-700">{verificationRequest.adminNotes}</p>
                <div className="mt-4 space-y-3">
                  <input value={evidenceLabel} onChange={(event) => setEvidenceLabel(event.target.value)} placeholder={t("artistDashboard.edit.evidenceLabelPlaceholder")} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" />
                  ...
                  <textarea value={evidenceNotes} onChange={(event) => setEvidenceNotes(event.target.value)} rows={3} placeholder={t("artistDashboard.edit.evidenceNotesPlaceholder")} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" />
                  <button onClick={handleEvidenceSubmit} className="w-full rounded-lg bg-amber-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-amber-700">
                    {t("artistDashboard.edit.submitEvidence")}
                  </button>
                </div>
              </section>
            )}
            <section className="rounded-xl border border-gray-100 bg-white p-6">
              <h2 className="mb-4 font-semibold text-gray-900">{t("artistDashboard.edit.pricing")}</h2>
              ...
              <p className="mt-2 text-sm text-gray-500">{t("artistDashboard.edit.rwfListingPrice")}</p>
            </section>
            {artwork.latestPricingRecommendation && (
              <section className="rounded-xl border border-purple-100 bg-white p-6">
                <h2 className="mb-4 font-semibold text-gray-900">{t("artistDashboard.edit.priceRecommendation")}</h2>
                <div className="grid grid-cols-3 gap-2 text-center text-sm">
                  <div className="rounded-lg bg-gray-50 p-2">
                    <p className="text-xs text-gray-500">{t("artistDashboard.edit.min")}</p>
                  ...
                  <div className="rounded-lg bg-purple-50 p-2 text-purple-700">
                    <p className="text-xs">{t("artistDashboard.edit.suggested")}</p>
                    <p className="font-semibold">{artwork.latestPricingRecommendation.suggested.toLocaleString()}</p>
                  </div>
                  <div className="rounded-lg bg-gray-50 p-2">
                    <p className="text-xs text-gray-500">{t("artistDashboard.edit.max")}</p>
            ...
            {artwork.latestImpactEstimate && (
              <section className="rounded-xl border border-green-100 bg-white p-6">
                <h2 className="mb-4 font-semibold text-gray-900">{t("artistDashboard.edit.impactEstimate")}</h2>
                <div className="space-y-2 text-sm text-gray-700">
                  <p><strong>{artwork.latestImpactEstimate.kgDiverted.toFixed(1)} kg</strong> {t("artistDashboard.edit.diverted")}</p>
                  <p><strong>{artwork.latestImpactEstimate.co2eAvoidedKg.toFixed(1)} kg</strong> {t("artistDashboard.edit.co2eAvoided")}</p>
                  <p><strong>{artwork.latestImpactEstimate.landfillVolumeAvoidedLitres.toFixed(1)} L</strong> {t("artistDashboard.edit.landfillVolumeAvoided")}</p>
                  <p className="capitalize text-gray-500">{t("artistDashboard.edit.confidence", { value: artwork.latestImpactEstimate.confidence })}</p>
                </div>
              </section>
            )}
            <section className="rounded-xl border border-gray-100 bg-white p-6">
              <h2 className="mb-4 font-semibold text-gray-900">{t("artistDashboard.edit.status")}</h2>
              ...
              <Link href={`/artwork/${artwork.slug}`} target="_blank" className="mt-4 inline-flex text-sm font-medium text-teal-700">
                {t("artistDashboard.edit.viewPublicListing")}
              </Link>
            </section>
          </aside>
        </div>
