...
export default function OrderArtworkInfoPage() {
  ...
  useEffect(() => {
    ...
    readArtwork(params.slug)
      ...
      .catch(() => active && setError("Could not load this artwork."))
    ...
    <div className="min-h-screen bg-gray-50">
      <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
        ...
        {loading ? (
          ...
          <p className="mt-8 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">{error || "Artwork not found."}</p>
        ) : (
          <article className="mt-6 overflow-hidden rounded-xl border border-gray-200 bg-white">
            ...
            <div className="p-6">
              ...
              <p className="mt-6 text-sm text-gray-500">{artwork.kgDiverted.toFixed(1)} kg of waste diverted</p>
            </div>
          </article>
        )}

        <p className="mt-4 text-center text-xs text-gray-400">Information only — this page has no purchase options.</p>
      </main>
    </div>
  );
}
