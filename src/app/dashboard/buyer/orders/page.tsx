    ...
    <DashboardLayout role="buyer" userName="Buyer">
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-500">Track orders managed by RenewCanvas from payment to delivery</p>
        </div>
        ...
        <div className="flex items-start gap-3 rounded-xl border border-blue-100 bg-blue-50 p-4">
          ...
          <p className="text-sm text-blue-700">Buyers pay RenewCanvas Africa directly. Admins coordinate order, delivery, and return communication.</p>
        </div>
        ...
        <div className="space-y-4">
                  ...
                  <div className="border-t border-gray-100 bg-gray-50 p-4">
                    <div className="grid gap-4 md:grid-cols-2">
                      <div className="space-y-2 text-sm text-gray-700">
                        ...
                        <p className="flex items-center gap-2 text-green-600"><Recycle className="h-4 w-4" />{item?.kgDiverted.toFixed(1) ?? "0.0"} kg diverted</p>
                        <p className="flex items-start gap-2"><MapPin className="mt-0.5 h-4 w-4" />{String(delivery.address ?? "")}, {String(delivery.city ?? "")}</p>
                      </div>
                      <div className="space-y-2">
                        <Link href={`/order-confirmation?order=${encodeURIComponent(order.id)}`} className="block rounded-lg bg-teal-600 px-4 py-2.5 text-center text-sm font-medium text-white">Payment Instructions</Link>
                        {item?.artworkSlug && <Link href={`/artwork/${item.artworkSlug}`} className="block rounded-lg border border-gray-200 px-4 py-2.5 text-center text-sm font-medium text-gray-700">View Artwork</Link>}
                      </div>
                    </div>
                  </div>
                )}
          ...
          {filteredOrders.length === 0 && (
            <div className="rounded-xl border border-gray-100 bg-white p-12 text-center">
              ...
              <Link href="/marketplace" className="mt-4 inline-flex rounded-lg bg-teal-600 px-4 py-2 text-white">Browse Marketplace</Link>
            </div>
          )}
        </div>
      </div>
